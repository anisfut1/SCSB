 ...
 * club (§27 du brief SaaS) : l'admin d'un club ne voit que ses propres
 * runs, jamais ceux d'un autre club de la plateforme.
 */
export default async function SyncDashboardPage({ params }: { params: Promise<{ clubSlug: string }> }) {
  ...
  const [{ data: recentSyncRuns }, { data: matches }, { data: recentImports }] = await Promise.all([
    supabase.from("sync_runs").select("*").eq("club_id", club.id).order("started_at", { ascending: false }).limit(10),
    supabase.from("matches").select("id, numero, emarque_status").eq("club_id", club.id),
    supabase.from("emarque_imports").select("*").eq("club_id", club.id).order("created_at", { ascending: false }).limit(10),
  ]);
    ...
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="mt-1 text-sm text-black/60 dark:text-white/60">
          ...
          les jobs tournent seuls (cron), sans intervention nécessaire ici.
        </p>
      </div>
