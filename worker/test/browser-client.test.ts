...
import { TestServer, readFixture } from "./support/static-server.js";
import { BrowserFbiClient } from "../src/fbi/browser-client.js";
import { FbiError } from "../src/fbi/errors.js";

const dirname = path.dirname(fileURLToPath(import.meta.url));
const fixture = (name: string) => readFixture(path.join(dirname, "fixtures", name));
...
describe("BrowserFbiClient.login (contre un serveur HTML local synthétique, jamais le vrai FBI)", () => {
  ...
  it("isole chaque session dans son propre contexte navigateur (jamais de cookie partagé entre deux connexions, §13 du brief FBI)", async () => {
    const client = new BrowserFbiClient({ baseUrl: server.baseUrl, browser, navigationSettleMs: 50 });
...
describe("BrowserFbiClient.findEmarqueDocuments (navigation générique, best-effort)", () => {
  it("navigue jusqu'à l'écran de résultat et trouve les liens de documents connus", async () => {
    ...
    // Le lien "Voir le règlement" ne doit jamais être confondu avec un document e-Marque.
    expect(documents.some((d) => d.url.includes("autre-lien-sans-rapport"))).toBe(false);

    await client.closeSession(session);
  });

  it("renvoie une liste vide (jamais une erreur) quand aucun document n'est encore disponible — §27 du brief FBI", async () => {
...
describe("BrowserFbiClient.downloadDocument", () => {
  it("télécharge le contenu via le contexte authentifié, sans passer par le système de fichiers (§20/§21 du brief FBI)", async () => {
    const client = new BrowserFbiClient({ baseUrl: server.baseUrl, browser, navigationSettleMs: 50 });
    const session = await client.login({ username: "club1234", password: "secret" });
