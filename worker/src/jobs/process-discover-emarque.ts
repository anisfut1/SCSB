...
import { BrowserFbiClient, type BrowserFbiSession } from "../fbi/browser-client.js";
import { classifyFbiLoginStatus, FbiError } from "../fbi/errors.js";
import { inferMatchDocumentType, mimeTypeForFileName } from "../fbi/document-type.js";
import { emarqueStoragePath, resolveSeasonLabel, uploadEmarqueFile } from "../storage.js";
import { nextErrorBackoffSeconds, nextWaitingBackoffSeconds } from "../backoff.js";
import { logError, logInfo } from "../logger.js";

const UNIQUE_VIOLATION = "23505";

async function rescheduleJob(supabase: WorkerSupabaseClient, job: FbiJobRow, delaySeconds: number, lastError: string | null): Promise<void> {
  await supabase
    .from("fbi_jobs")
...
async function failJob(supabase: WorkerSupabaseClient, job: FbiJobRow, message: string): Promise<void> {
  await supabase.from("fbi_jobs").update({ status: "failed", finished_at: new Date().toISOString(), last_error: message }).eq("id", job.id);
}

async function recordLoginOutcome(supabase: WorkerSupabaseClient, clubId: string, success: boolean, message: string | null): Promise<void> {
 ...
 * puis une ligne `match_documents` PAR fichier (§19/§23 du brief FBI). Le
 * PARSING du ZIP n'a pas lieu ici — voir
 * src/lib/domain/emarque/parse-downloaded-documents.ts (cron Vercel léger,
 * sans Playwright) qui consomme `match_documents.status = 'downloaded'`.
 */
export async function processDiscoverEmarqueJob(
  ...
  try {
    ...
    if (status === "INVALID_CREDENTIALS" || status === "AUTH_FLOW_CHANGED") {
      // Ne se corrigera jamais tout seul en réessayant (§15 du brief FBI) —
      // FBI_AUTOMATION_CHANGED est surfacé via fbi_integration_status.last_error,
      // visible sur /c/{slug}/admin/integrations (§44).
  ...
  try {
    ...
    // Priorité absolue au ZIP complet s'il existe (§19) : les documents
    // séparés ne sont téléchargés que si aucun ZIP n'est disponible.
